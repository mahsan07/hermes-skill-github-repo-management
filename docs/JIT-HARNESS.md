...
This package expresses **GitHub Repository Management** as a typed, task-adaptive agent harness. Its concrete capability remains: Clone, create, fork, and manage GitHub repositories, remotes, and releases. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in GitHub Repository Management |
| --- | --- | --- |
| Memory | `resource_state_registry` | Retain repository intent plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `resource_orchestration_plan` | Transform local git state into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_preview_authorize_execute_reread` | Advance the workflow through github repository api while preserving stop conditions and user authority. |
